...
- **Chat advisor** — a conversational RAG assistant that explains syllabi and learning paths, grounded in the catalog. It keeps conversational memory: retrieval blends the recent turns so a follow-up like "what's next?" stays on topic, and the thread persists across page reloads. Every course it (or a roadmap) surfaces is clickable for a summary, skills, and a link to the real course.
...
- **Career-track roadmaps** — 17 tracks across Data & AI, Software & Cloud, Business & Product, and Design (Data Scientist, Cloud Engineer, Cybersecurity, Business/Financial Analyst, Digital Marketing, UX…), each a tiered Foundation→Specialization roadmap grounded in real catalog courses with bridges to adjacent tracks. The picker only shows tracks the loaded catalog actually covers, so it reflects the data and grows with it.
